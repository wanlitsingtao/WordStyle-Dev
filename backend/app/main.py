...
def run_migrations():
    ...
    try:
        ...
        if database_url:
            ...
            logger.info("[OK] Alembic 使用环境变量 DATABASE_URL")
        
        # [OK] 修复：直接执行 upgrade，不使用 stamp
        # stamp 会将数据库标记为最新版本，导致 upgrade 跳过所有迁移
        # 如果有多 head 冲突，Alembic 会抛出明确错误，需要手动解决
        logger.info("[REFRESH] 执行数据库迁移...")
        command.upgrade(alembic_cfg, "head")
        logger.info("[OK] 数据库迁移完成")
        
    except Exception as e:
        logger.error(f"[ERROR] 数据库迁移失败: {e}")
...
def create_application() -> FastAPI:
    ...
    @application.api_route("/health", methods=["GET", "POST", "HEAD"])
    def health_check():
...
def startup_event():
    """应用启动时执行的任务"""
    logger.info(f"[LAUNCH] {settings.APP_NAME} v{settings.APP_VERSION} 启动中...")
    run_migrations()
