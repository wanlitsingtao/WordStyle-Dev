...
def render_conversion_config():
    ...
    提示语布局完全参照桌面版：
    - 第1行：复选框 + 类型选择（文本/图片）横向排列
    - 第2行：提示语文本输入框（始终显示，图片模式灰显）
    - 第3行：图片文件选择（始终显示，文本模式灰显）
    - 第4行：提示语样式 + 设为默认按钮
    - 未勾选时：仅显示复选框，不显示设为默认按钮
    """
    # CSS：统一控件高度
    st.markdown("""
    <style>
    ...
    # 第1行：复选框 + 类型选择（参照桌面版：同一行，横向排列）
    hint_row1 = st.columns([1, 2])
    ...
    if do_hint:
        # 第2行：提示语文本输入框（参照桌面版：始终显示，图片模式灰显）
        hint_row2 = st.columns([1, 4])
        with hint_row2[0]:
            st.markdown("提示语文本:")
        with hint_row2[1]:
        ...
        # 第4行：提示语样式 + 设为默认按钮（参照桌面版）
        hint_row4 = st.columns([1, 2, 1])
        with hint_row4[0]:
            st.markdown("提示语样式:")
        with hint_row4[1]:
        ...
        # 未勾选时：不显示任何额外控件（不显示"设为默认"按钮）
        pass
        # 确保 hint_image_path 变量存在
